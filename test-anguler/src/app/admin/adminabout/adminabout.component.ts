...
import { AdminAboutService } from './adminabout.service';
...
@Component({
    ...
    styleUrls: ['./adminabout.component.scss'],
...
export class AdminAboutComponent {
    ...
    imageSrc: any;
    constructor(private _appConfig: AppConfig, private _adminaboutservice: AdminAboutService, private router: Router, vcRef: ViewContainerRef, fb: FormBuilder, public _isAdmin: PagesComponent) {
    ...
    ngOnInit() {
        ...
        this._adminaboutservice.getFileContent().subscribe(
            result => {
                this.ckeditorContent = result.data[0].configData;
                // console.log("result", this.ckeditorContent);
            }
        )
        this._adminaboutservice.getImages().subscribe(
            result => {
    ...
    confirmUpload() {
        ...
        this._adminaboutservice.gotoimageUpload(dataURI).subscribe(
            result => {
    ...
    gotoSaveFile() {
        ...
        this._adminaboutservice.gotoSubmitFile(this.ckeditorContent, this.type).subscribe(
            result => {
                // console.log("result", result);
                this.ngOnInit();
                if (result.code == 200) {
